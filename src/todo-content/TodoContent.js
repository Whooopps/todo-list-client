...
function TodoContent() {
    ...
    const [items, setItems] = useState({
        1: {name: 'Shopping list', items: [ { id: 1, name: 'Onions Onions Onions Onions Onions Onions Onions Onions Onions Onions Onions Onions Onions Onions Onions Onions', completed: false}, { id: 2, name: 'Gosht', completed: true }, { id: 3, name: 'Potatoes', completed: true}, { id: 4, name: 'Potatoes', completed: true}, { id: 5, name: 'Potatoes', completed: true}, { id: 6, name: 'Potatoes', completed: true}, { id: 7, name: 'Potatoes', completed: true}, { id: 8, name: 'Potatoes', completed: true}, { id: 9, name: 'Potatoes', completed: true}, { id: 10, name: 'Potatoes', completed: true} ]},
    ...
    const {isEditing: editingHeader, ref:headerRef, value: headerValue, bindInput, bindEl, cancelEditing, startEditing} = useEditable(items[query.listId] ? items[query.listId].name : '', endHeaderEditing);
    ...
    }, [query.listId, location.state?.isNew]);
    ...
    useListener(Event.NEW_LIST_ITEM_UPDATED, useCallback((item) => {
        if (items[query.listId]) {
            setItems({
                ...items,
                [query.listId]: {
    ...
    useListener(Event.LIST_ITEM_DELETED, useCallback((id) => {
        if (items[query.listId]) {
            setItems({
                ...items,
                [query.listId]: {
    ...
    useListener(Event.LIST_DELETED, useCallback((listId) => {
    ...
    function renderItems(listId) {
        let newExpectedId = items[listId].items.reduce((mx, item) => Math.max(mx, item.id), 0) + 1;
        return [<TodoListItem key={`${listId}-${0}`} id={newExpectedId} isNew={true} shouldFocus={!editingHeader} />].concat(
            ...
            <div className="flex-grow mt-10 px-44">
                {query.listId && items[query.listId] ? (
                    <Fragment>
                        ...
                        (<h2 className="text-center text-3xl text-bold mb-4 pb-2 border-b border-transparent" {...bindEl}>{headerValue}</h2>)}
                        { items[query.listId].items.length > 0 ? (
                            <div className="text-right mb-6">
                                <button className={"bg-red-500 text-white rounded text-center px-5 py-1.5 disabled:bg-red-200" } onClick={clearAll}><i className="fas fa-trash mr-2"></i>Clear All</button>
                            </div>
                        ) : null }
