...
function TodoListItem({ id, name, completed, isNew, shouldFocus = true }) {
    ...
    const { isEditing, ref, value, bindInput, bindEl } = useEditable(name, editItem);
    const dispatcher = useDispatch();
    const newItemInputRef = useCallback((input) => {
            ...
            <div className="flex items-start overflow-hidden">
                {isNew ? (
                    ...
                    <Fragment>
                        ...
                        />) :
                            (<Fragment>
                                <p className={`text-lg flex-grow ${checked ? 'line-through' : ''}`} {...bindEl}>{value}</p>
                                <div className="flex-grow-0 flex-shrink-0 w-6 h-6 self-center transform translate-x-2x transition-all group-hover:translate-x-0">
                                    <a href="#" onClick={onDeleteClicked}><i className={"fas fa-trash text-red-800"}></i></a>
                                </div>
                            </Fragment>)
                        }
                    </Fragment>
